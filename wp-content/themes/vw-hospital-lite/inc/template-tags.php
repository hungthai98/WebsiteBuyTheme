...
function vw_hospital_lite_categorized_blog() {
	if ( false === ( $vw_hospital_lite_all_the_cool_cats = get_transient( 'vw_hospital_lite_all_the_cool_cats' ) ) ) {
		// Create an array of all the categories that are attached to posts
		$vw_hospital_lite_all_the_cool_cats = get_categories( array(
			'hide_empty' => 1,
		) );

		// Count the number of categories that are attached to the posts
		$vw_hospital_lite_all_the_cool_cats = count( $vw_hospital_lite_all_the_cool_cats );

		set_transient( 'vw_hospital_lite_all_the_cool_cats', $vw_hospital_lite_all_the_cool_cats );
	}

	if ( '1' != $vw_hospital_lite_all_the_cool_cats ) {
...
function vw_hospital_lite_category_transient_flusher() {
	// Like, beat it. Dig?
	delete_transient( 'vw_hospital_lite_all_the_cool_cats' );
}
 ...
 * Posts pagination.
 */
if ( ! function_exists( 'vw_hospital_lite_blog_posts_pagination' ) ) {
	function vw_hospital_lite_blog_posts_pagination() {
		$pagination_type = get_theme_mod( 'vw_hospital_lite_blog_pagination_type', 'blog-page-numbers' );
